from PyQt6.QtWidgets import (
    ...
    QStatusBar, QHeaderView, QDialog, QInputDialog, QGroupBox, QFormLayout, QHBoxLayout, QComboBox
)
...
class RestHouseApp(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        export_action = QAction("Export Guests to CSV", self)
        export_action.setToolTip("Export guests table to CSV")
        export_action.triggered.connect(lambda: self.export_guests_csv())
    ...
    def export_guests_csv(self):
        try:
            self.manager.export_to_csv("guests", "guests.csv")
            self.show_info("Guests exported to guests.csv successfully.")
        except Exception as e:
            self.show_error(f"Error exporting guests: {e}")

    def show_about(self):
        QMessageBox.information(self, "About", "Rest House Management System v1.0")
    ...
    def is_valid_name(self, name):
        ...
        # Example: allow letters, digits, and require length 2 to 50
        # This uses a regex that allows alphabets and digits only.
    ...
    def add_guest_tab(self):
        ...
        title_label = QLabel("Register a New Guest")
        title_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        main_layout.addWidget(title_label)

        # Group box for guest details
        guest_group = QGroupBox("Guest Information")
        guest_form_layout = QFormLayout()
        guest_form_layout.setSpacing(10)
        guest_form_layout.setLabelAlignment(Qt.AlignmentFlag.AlignRight)

        self.guest_name_input = QLineEdit()
        self.guest_name_input.setPlaceholderText("Enter guest's full name")
        guest_form_layout.addRow(QLabel("Guest Name:"), self.guest_name_input)

        self.contact_input = QLineEdit()
        self.contact_input.setPlaceholderText("Enter phone number or email")
        guest_form_layout.addRow(QLabel("Contact:"), self.contact_input)

        guest_group.setLayout(guest_form_layout)
        main_layout.addWidget(guest_group)
    ...
    def make_reservation_tab(self):
        ...
        title_label = QLabel("Create a New Reservation")
        title_label.setStyleSheet("font-size: 16px; font-weight: bold;")
        layout.addWidget(title_label)

        # GroupBox for reservation details
        reservation_group = QGroupBox("Reservation Details")
        reservation_layout = QFormLayout()
        reservation_layout.setSpacing(10)
        reservation_layout.setLabelAlignment(Qt.AlignmentFlag.AlignRight)

        self.guest_combo = QComboBox()
        guests = self.manager.list_guests()
        for guest_id, guest_name, contact in guests:
            self.guest_combo.addItem(f"{guest_name} (ID: {guest_id})", guest_id)
        reservation_layout.addRow(QLabel("Select Guest:"), self.guest_combo)

        self.room_combo = QComboBox()
        rooms = self.manager.get_available_rooms()
        for room_id, room_number, room_type, status in rooms:
            self.room_combo.addItem(f"{room_number} ({room_type})", room_id)
        reservation_layout.addRow(QLabel("Select Room:"), self.room_combo)

        self.check_in_input = QLineEdit()
        self.check_in_input.setPlaceholderText("e.g., 2023-11-10")
        reservation_layout.addRow(QLabel("Check-In Date:"), self.check_in_input)

        self.check_out_input = QLineEdit()
        self.check_out_input.setPlaceholderText("e.g., 2023-11-12")
        reservation_layout.addRow(QLabel("Check-Out Date:"), self.check_out_input)
        ...
        make_reservation_button.setToolTip("Click to finalize reservation")
        # make_reservation_button.setIcon(QIcon(":/icons/book_room.png"))
        make_reservation_button.clicked.connect(self.make_reservation)
        button_layout.addWidget(make_reservation_button)

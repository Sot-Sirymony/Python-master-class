...
#     QStatusBar, QHeaderView, QDialog, QInputDialog, QGroupBox, QFormLayout, QHBoxLayout
# )
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout, QLabel,
    QLineEdit, QPushButton, QTableWidget, QTableWidgetItem, QMessageBox,
...
class RestHouseApp(QMainWindow):
    ...
    def add_guest_tab(self):
        ...
        # Group box to hold guest form
        guest_group = QGroupBox("New Guest Details")
        guest_group_layout = QFormLayout()
        guest_group_layout.setSpacing(10)
        guest_group_layout.setLabelAlignment(Qt.AlignmentFlag.AlignRight)

        self.guest_name_input = QLineEdit()
        self.guest_name_input.setPlaceholderText("Enter guest's full name")
        guest_group_layout.addRow(QLabel("Guest Name:"), self.guest_name_input)

        self.contact_input = QLineEdit()
        self.contact_input.setPlaceholderText("Enter phone number or email")
        guest_group_layout.addRow(QLabel("Guest Contact:"), self.contact_input)

        guest_group.setLayout(guest_group_layout)
        ...
        # add_guest_button.setIcon(QIcon(":/icons/add_user.png"))  # Replace or remove if you don't have icons
        add_guest_button.setToolTip("Click to add new guest to the database")
    ...
    def make_reservation_tab(self):
        ...
        # Populate the room combo box with available rooms
        rooms = self.manager.get_available_rooms()  # returns (id, room_number, type, status)
    ...
    def make_reservation(self):
        # Retrieve guest_id and room_id from combo boxes rather than QLineEdit
